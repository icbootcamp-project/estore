...
const Categories = ({ categories, switchCategoriesActive, switchCategoriesRight, switchCategoriesLeft }) => {
  function renderCategories() {
    if (window.innerWidth === 750) {
      return categories
        .slice(0, 5)
        .map((item) => <Category key={item.id} category={item} switchCategoriesActive={switchCategoriesActive} />);
    ...
    <Fragment>
      <div id="categories">
        <div className="category-item">
          <a role="button" tabIndex={0} onKeyPress={() => handleClick('left')} onClick={() => handleClick('left')}>
            <img
          ...
          <div className="category-items-container">{renderCategories()}</div>
          <a role="button" tabIndex={0} onKeyPress={() => handleClick('right')} onClick={() => handleClick('right')}>
            <img
