...
const array = ['small', 'large', 'extra-large', 'extreme-large', 'veryMuch Extra large'];
const renderData = array.map((item) => (
  <div key={array.indexOf(item)}>
    <label className="modal-selection-text" htmlFor={item}>
      <div>{item}</div>

      <input className="modal-div-radio-buttons" type="radio" id={item} name="drone" value={item} />
    </label>
  </div>
));
    ...
    return (
      <div className="center-component-for-modal-details">
        <div className="to-display-two-things upper-portion-modal-div">
          ...
          <button type="button" className="close close-button-for-modal" aria-label="Close" onClick={closeModal}>
